...
app.post('/api/products', upload.array('images', 6), async (req, res) => {
    try {
        ...
        // Check if Vercel Blob access configuration is present
        if (process.env.BLOB_READ_WRITE_TOKEN) {
            ...
            // LOCAL DEVELOPMENT FALLBACK: Bypasses Vercel Cloud so local saves don't break
            console.log("⚠️ Missing BLOB_READ_WRITE_TOKEN. Using local placeholder image URLs.");
            for (const file of req.files) {
